...
function hhWrite(year, month){
	...
	if(!category || !content || !price || !type ){ // 여기까지 했음.
		...
		if(year != date.substring(0,4) || month !=date.substring(5,7)){
			...
			httpRequest.open("POST", "http://13.209.93.8:8081/Project_semi/hhWriteController",true);
...
function hhCategoryWrite(year, month){
	...
	if(!category ){ // 여기까지 했음.
		...
		return false;
			...
			httpRequest = getXMLHttpRequest();
			httpRequest.onreadystatechange = checkFunc;
			httpRequest.open("POST", "http://13.209.93.8:8081/Project_semi/hhWriteCatController",true);
			httpRequest.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded;charset=UTF-8');
			httpRequest.send(param);
		}
	}
...
function categoryDelete(){
	...
	if(delConfirm ){
			...
			httpRequest = getXMLHttpRequest();
			httpRequest.onreadystatechange = checkFunc;
			httpRequest.open("POST", "http://13.209.93.8:8081/Project_semi/hhDeleteCategory",true);
			httpRequest.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded;charset=UTF-8');
			httpRequest.send(param);
	}
	else{
